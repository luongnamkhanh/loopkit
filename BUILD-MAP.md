...
| Thread follow-ups (`message.channels`) | ✅ ❗ | reply with `DoD:` in an owned thread → new run seeded with previous artifact (revision base). Needs Slack scopes `channels:history`(+groups) + `message.channels` events + reinstall. Known edges: reply containing any `<@mention>` is ceded to app_mention; dead if ENABLE_MEMORY off. **Gap found live (refinement E2E): DM surface KHÔNG được cover — token thiếu `im:history` + event `message.im` → mọi reply thường trong DM rơi vào hư không (mention vẫn tới vì `app_mentions:read`). Fix = thêm scope `im:history` + event `message.im` + reinstall; code không cần đổi (handler channel-type agnostic)** |
