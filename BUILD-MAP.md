...
| REAL gate: EARS → pytest | ✅ | `gates.py`: explicit `Tests:` (AST-validated) > derived-from-DoD (fresh call BEFORE generation, frozen) > compile-only fallback (warned). |
| Domain gate: helm/kubeconform variants (edit-in-place) | 🟡 | spec 2026-07-13: `Gate: <lệnh shell>` (`gates.parse_gate_cmd`/`make_cmd_gate`) → edit-mode (`engine.py` `Ticket.gate_cmd`): generator sửa thẳng worktree, gate = lệnh domain (helm template+lint / terraform validate / pytest / grep có chủ đích), artifact = git diff (`engine._worktree_diff`), delivery = `deliver.ship_diff` (commit toàn bộ thay đổi, không move file). `REPOS_PENDING` (repo pending/hạ tầng chưa có gate riêng) → `deliver.infer_gate` tự đề xuất Gate: trước khi fail-closed. Traces back tới ticket savepoint-upgradeMode 2026-07-09 (helm upgrade mất data, phải fix tay). **✅ live E2E 2026-07-13: ticket edit-mode thật từ Telegram (front-enrichment, đúng ca issue #1) → gate=repo's own pytest → door diff → approve từ phone → gh PR #3 TỰ ĐỘNG. Nhánh MR-thật chạy live lần đầu** |
...
| `Repo:` routing đa repo | ✅ | spec 2026-07-09: allowlist `LOOPKIT_REPOS` (unknown → từ chối kèm danh sách TRƯỚC mọi LLM call); ws_key=`<repo>-<thread>` (đổi repo giữa thread không dính worktree cũ); context per-ticket (repo-mode đọc AGENTS.md repo đích); analyst nhận danh sách repo để hỏi & điền; không `Repo:` → `TARGET_REPO` (compat). **`REPOS_PENDING` semantics đổi ở spec 2026-07-13: KHÔNG còn nghĩa "chờ domain gate" (từ chối cứng) — nay là "requires-gate": `deliver.infer_gate` tự suy Gate: trước, chỉ từ chối khi infer thất bại (None)** |
...
## 7d · Sự cố nghiệm thu 2026-07-13 (fractal loop) — vá NÓNG
| Item | Status | Note |
|---|---|---|
| Gate cấm brain (`LOOPKIT_NO_BRAIN`) | ✅ | live incident: generator viết test KHÔNG hermetic gọi run_loop thật → gate pytest spawn claude lồng nhau đệ quy (8+ process, tự nhân bản). Fix: mọi gate subprocess (cmd/pytest/place_and_verify) chạy với `LOOPKIT_NO_BRAIN=1`; `ask_claude`/`run_agent` short-circuit trả sentinel → test ẩu chết ngay trong giây, feedback dạy generator mock. 3 tests |
