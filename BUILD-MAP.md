...
| Delivery: MR link sau approve | ✅ | spec 2026-07-10: `Deliver:` AI điền lúc freeze (`infer_path`; analyst-proposed-in-draft ⬜ defer — freeze-time inference phủ mọi flow), người phủ-quyết tại door; `deliver.ship` sau human door: move→re-gate→`feat/<module>`→push→MR (glab/gh, fallback link từ push output); resume path re-materialize worktree từ door payload; fail = báo rõ + giữ local, không rollback approve. Live E2E còn chờ (GitLab group hết read-only) |
