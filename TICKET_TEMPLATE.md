...
## Gate (edit-in-place — thường để AI điền)
`Gate: <lệnh shell>` — có nó là generator sửa thẳng repo (không tạo `solution.py`), gate =
lệnh shell chạy trong worktree, artifact = diff. Bắt buộc với repo `REPOS_PENDING` (hạ tầng
chưa có gate riêng) — thiếu thì loopkit tự suy ra (AI đề xuất) hoặc từ chối. `Gate:` và
`Deliver:` loại trừ nhau (`Gate:` thắng, `Deliver:` bị bỏ qua kèm cảnh báo). Gate: phải đứng
TRƯỚC DoD: — đứng sau sẽ bị bỏ qua im lặng (parser chỉ tìm token thật trước DoD).
