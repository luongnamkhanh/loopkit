...
| Domain gate: helm/kubeconform variants (edit-in-place) | 🟡 | spec 2026-07-13: `Gate: <lệnh shell>` (`gates.parse_gate_cmd`/`make_cmd_gate`) → edit-mode (`engine.py` `Ticket.gate_cmd`): generator sửa thẳng worktree, gate = lệnh domain (helm template+lint / terraform validate / pytest / grep có chủ đích), artifact = git diff (`engine._worktree_diff`), delivery = `deliver.ship_diff` (commit toàn bộ thay đổi, không move file). `REPOS_PENDING` (repo pending/hạ tầng chưa có gate riêng) → `deliver.infer_gate` tự đề xuất Gate: trước khi fail-closed. Traces back tới ticket savepoint-upgradeMode 2026-07-09 (helm upgrade mất data, phải fix tay). **unit-verified (161 tests); live E2E (ticket thật vào streaming-deploy từ Telegram) PENDING — flip ✅ khi nghiệm thu** |
