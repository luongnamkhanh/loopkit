...
| Delivery: MR link sau approve | ⬜ ❗ | demanded live ×2: normalize_phone 2026-07-08 và bearing_deg 2026-07-09/10 — đều giao TAY (move theo pattern → commit → push `feat/<x>` → MR); lần 2 lộ thêm: worktree `/tmp` bay sau reboot (cứu bằng semantic cache) + push fail GitLab read-only phải báo rõ. **Spec approved 2026-07-10** (`docs/superpowers/specs/2026-07-10-delivery-automation-design.md`): `Deliver:` do AI điền lúc freeze, người phủ-quyết tại door; deliver deterministic move→re-gate→commit→push→MR (glab/gh, fallback link) |
